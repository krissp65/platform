...
func generateRoutes(entries ...HandlerEntry) []Route {
	...
	for _, entry := range entries {
		...
		promoteMethods := getAnonymousFieldMethods(handlerType)
		for i := 0; i < handlerType.NumMethod(); i++ {
			method := handlerType.Method(i)
			methodName := strings.ToUpper(method.Name)
			for _, httpMethod := range httpMethods {
				if strings.Index(methodName, httpMethod) == 0 {
					if matchesPromoteMethodName(method, promoteMethods) {
						continue
					}
					route := Route{
...
func matchesPromoteMethodName(method reflect.Method, methods []reflect.Method) bool {
...
func generateRegularExpression(prefix string, route *Route) {
	...
	if route.httpMethod == http.MethodGet {
		for i := 1; i < int(route.handlerMethod.Type.NumIn()); i++ {
